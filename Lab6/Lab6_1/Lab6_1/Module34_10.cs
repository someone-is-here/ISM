namespace Lab6 {
    class Module34_10 {
        ...
        public Module34_10(BigInteger p, BigInteger a, BigInteger b, BigInteger n, byte[] xG) {
            this.a = a;
            this.b = b;
            this.n = n;
            this.p = p;
        ...
        public Point genPublicKey(BigInteger d) {
            Point G=gDecompression();
            Point Q = Point.multiply(G, d);
            return Q;
        }

        //восстановление координат Y из координаты X и бита четности Y.
        private Point gDecompression() {
            byte y = xG[0];
        ...
        public bool verifDS(byte[] H, string sign, Point Q) {
            ...
            Point A = Point.multiply(G, z1);
            Point B = Point.multiply(Q, z2);
            Point C = A + B;
            BigInteger R = C.x % n;
            if (R == r)
                return (true);
